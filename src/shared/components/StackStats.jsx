...
const StackStats = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6">
      {loading ? (
        <div className="max-w-screen bg-sky-200 flex mx-auto p-4 text-slate-900 sm:p-6 rounded-lg border border-sky-500">
          <svg
            aria-hidden="true"
            class="mr-2 w-8 h-8 text-white animate-spin fill-green-600"
            viewBox="0 0 100 101"
            fill="none"
        ...
        <div className="max-w-screen bg-sky-200 flex flex-col mx-auto p-4 text-slate-900 sm:p-6 rounded-lg border border-sky-500">
          <h3 className="text-lg font-semibold text-slate-900 sm:text-xl">
            {profile.display_name + "'s Stack Overflow Stats"}
          </h3>
          <div className="flex flex-col sm:flex-col space-y-2">
            <p>
              {"I've been awarded " +
                profile.badge_counts.gold +
                " gold badges, " +
                profile.badge_counts.silver +
                " silver badges and " +
                profile.badge_counts.bronze +
                " bronze badges, for a total of " +
                profile.reputation +
                " reputation."}
            </p>
            <a className="font-semibold" href={profile.link}>
              Visit my profile
            </a>
          </div>
        </div>
